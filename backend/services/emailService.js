...
const transporter = nodemailer.createTransport({
  host: process.env.EMAIL_HOST || 'mail.guidingstarszm.com',
  port: 587,
  secure: false,
  auth: {
...
transporter.verify((err, success) => {
  if (err) {
    ...
    console.warn('   Emails will not send until EMAIL_HOST, EMAIL_USER and EMAIL_PASSWORD are configured in .env');
  } else {
    ...
    html: `
      ...
      <head>
        <style>
          body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
          .container { max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f9f9f9; }
          .header { background-color: #2c3e50; color: white; padding: 30px; text-align: center; border-radius: 8px 8px 0 0; }
          .content { background-color: white; padding: 30px; border-radius: 0 0 8px 8px; }
          .footer { text-align: center; margin-top: 20px; color: #7f8c8d; font-size: 12px; }
        </style>
      </head>
      <body>
        <div class="container">
          <div class="header"><h1>Guiding Stars Application Update</h1></div>
          <div class="content">
            ...
            <p>We encourage you to apply again in the future. We wish you all the best in your endeavors.</p>
            <p>Best regards,<br><strong>The Guiding Stars Team</strong></p>
          </div>
          <div class="footer">
            <p>This is an automated message. Please do not reply to this email.</p>
...
const sendEmail = async (to, subject, htmlBody) => {
  try {
    if (!to || !subject || !htmlBody) {
      return { success: false, error: 'Missing required parameters: to, subject, htmlBody' };
    }
    const mailOptions = {
      from: process.env.EMAIL_USER,
      to,
      subject,
      html: htmlBody,
      replyTo: 'admissions@guidingstarszm.com'
    };
    await transporter.sendMail(mailOptions);
